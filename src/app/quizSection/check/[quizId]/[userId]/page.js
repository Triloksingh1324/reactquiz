    ...
    const fetchResponseAndQuestions = async () => {
      try {
        ...
        const responseRes = await axios.get(
          `/api/quiz/checkresponse?quizId=${quizId}&userId=${userId}`
        );
        const questionsRes = await axios.get(
          `/api/quiz/getquiz?quizId=${quizId}`
        );
    ...
    return (
      <>
        <div className="flex flex-col items-center justify-center min-h-screen ">
          <h2>This response has already been checked.</h2>
          <div className="p-4 bg-blue-400 hover:bg-blue-600 rounded-2xl mt-4" onClick={()=>{
            router.push("/")
          }}>Home</div>
        </div>
      </>
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">
        Responses for {response.userId.userName}
      </h1>
      <ul>
            ...
            <li key={index} className="mb-4">
              <div className="p-2 border rounded-md">
                  ...
                  <ul className="mb-2">
                    {question.options.map((option, idx) => (
                      <li key={idx} className="flex items-center">
                        ...
                        <span
                          className={`${
                            answer.answer === option.optionText
                              ? answerColor
                              : ""
                          } mr-2`}
                        >
                ...
                <p className="mb-2">
                  ...
                  <span className={answerColor}>{answer.answer}</span>
                </p>
                <p className="mb-2">
                  <strong>Score:</strong> {scores[answer.questionId]}
                </p>
                    ...
                    <label className="block">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleScoreChange(
                            answer.questionId,
                            Math.min(e.target.value, question.score)
                          )
                        }
                        max={question.score}
                        min="0"
